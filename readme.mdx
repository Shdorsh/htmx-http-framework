...
### Components
Components, or rather handlebars partials are parts that you can use in other components. The components are compiled when the server boots/refreshes. You can create them and insert them by using {{> PascalCaseFilePath}}. Let us try to make a component out of the counter. In ```view/components```, make a Counter.handlebars file with the following:
```html
  <div x-data="{count: {{count}} }">
    Current count: <span x-html="count"></span>
    <button x-bind="count" x-on:click="count++">Click me!</button>
